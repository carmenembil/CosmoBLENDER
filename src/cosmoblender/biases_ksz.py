...
class hm_framework:
    ...
    def __init__(self, lmax_out=500, m_min=1e10, m_max=5e15, nMasses=30, z_min=0.07, z_max=3, nZs=30, k_min = 1e-4,
                 ...
                 , xmax=5, nxs=40000, tsz_param_override={},y_press_profile='amodeo'):
        """ Inputs:
        ...
        # CEV: choose y profile 
        if y_press_profile == 'battaglia':
            self.hcos.add_battaglia_pres_profile("y",family="pres",xmax=xmax,nxs=nxs, param_override=self.tsz_param_override)
        elif y_press_profile == 'amodeo':
            self.hcos.add_amodeo20_gnfw_pres_profile("y",xmax=xmax,nxs=nxs)
        else:
            raise ValueError("y_press_profile must be either 'battaglia' or 'amodeo'. You entered " + y_press_profile)
